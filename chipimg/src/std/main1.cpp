...
struct Grid6x2Result {

    Point2f grid[6][2];
    bool    is_measured[6][2];
};

static float medianf(vector<float> v) {
...
static float estimate_row_step(const vector<Point2f>& col_pts, const vector<Point2f>& all_pts) {
    ...
    if (col_pts.size() >= 2) {
        ...
        for (size_t i=1;i<ys.size();++i) diffs.push_back(ys[i]-ys[i-1]);
    }
    if (diffs.size() < 1) {
    ...
    float step = medianf(diffs);
    if (!std::isfinite(step) || step <= 0.f) step = 25.f;
    return step;
}

static int row_from_y(float y, float y_anchor, float step) {
    float r = 5.f - (y_anchor - y) / step;
    int   ri = (int)lroundf(r);
    return std::max(0, std::min(5, ri));
}

static Grid6x2Result fitGrid6x2(const vector<Point2f>& pts,
                                const Point2f& anchor,
                                float same_col_dx = 30.f,
                                float first_col_shift = 90.f)
{
    Grid6x2Result out;
    for (int r=0;r<6;++r) for (int c=0;c<2;++c){
        out.grid[r][c] = Point2f(numeric_limits<float>::quiet_NaN(),
                                 numeric_limits<float>::quiet_NaN());
        out.is_measured[r][c] = false;
    }

    vector<Point2f> col0, col1;
    for (auto &p : pts) {
        if (std::fabs(p.x - anchor.x) <= same_col_dx) col1.push_back(p);
        else                                          col0.push_back(p);
    }

    float x_c1 = anchor.x;
    if (!col1.empty()){
        vector<float> xs; xs.reserve(col1.size());
        for (auto&p: col1) xs.push_back(p.x);
        x_c1 = medianf(xs);
    }
    float x_c0 = numeric_limits<float>::quiet_NaN();
    if (!col0.empty()){
        vector<float> xs; xs.reserve(col0.size());
        for (auto&p: col0) xs.push_back(p.x);
        x_c0 = medianf(xs);
    } else {

        if (std::isfinite(x_c1)) x_c0 = x_c1 - first_col_shift;
    }

    float step_c1 = estimate_row_step(col1, pts);
    float step_c0 = estimate_row_step(col0, pts);
    float step = (std::isfinite(step_c1) ? step_c1 : step_c0);
    if (!std::isfinite(step) || step<=0.f) step = 25.f;

    auto place_measured = [&](const vector<Point2f>& col_pts, int col){
        for (auto &p : col_pts){
            int r = row_from_y(p.y, anchor.y, step);
            if (!out.is_measured[r][col]) {
                out.grid[r][col] = p;
                out.is_measured[r][col] = true;
            } else {

                float y_target = anchor.y - (5 - r) * step;
                float old_d = std::fabs(out.grid[r][col].y - y_target);
                float new_d = std::fabs(p.y - y_target);
                if (new_d < old_d) out.grid[r][col] = p;
            }
        }
    };
    place_measured(col1, 1);
    place_measured(col0, 0);

    auto fill_missing = [&](int col, float xcol){
        if (!std::isfinite(xcol)) return;
        for (int r=0;r<6;++r){
            if (!out.is_measured[r][col]) {
                float y = anchor.y - (5 - r) * step;
                out.grid[r][col] = Point2f(xcol, y);
...
int main(int argc, char** argv)
{
    ...
    string path = (argc >= 2) ? string(argv[1]) : string("../Img/stdChip/C240501-zuidixiangyingzhi-01-20uor.png");
    ...
    const float kSameColDx = 30.f;
    const float kFirstColShift = 90.f;
    Mat vis_enh_bgr, vis_bin_bgr;
    cvtColor(view_enhanced, vis_enh_bgr, COLOR_GRAY2BGR);
    cvtColor(bin8,          vis_bin_bgr, COLOR_GRAY2BGR);

    for (size_t gi=0; gi<l2_points_with_idx.size(); ++gi) {
        const auto& pack = l2_points_with_idx[gi];
        vector<Point2f> pts_in_group; pts_in_group.reserve(pack.size());
        for (auto &pr : pack) pts_in_group.push_back(pr.first);

        const Point2f& anchor = anchors_l2[gi];
        if (!std::isfinite(anchor.x) || !std::isfinite(anchor.y)) continue;

        Grid6x2Result grid = fitGrid6x2(pts_in_group, anchor, kSameColDx, kFirstColShift);

        for (int r=0;r<6;++r){
            for (int c=0;c<2;++c){
                const Point2f& q = grid.grid[r][c];
                if (!std::isfinite(q.x) || !std::isfinite(q.y)) continue;

                if (grid.is_measured[r][c]) {

                    circle(vis_enh_bgr, q, kPtRadius, Scalar(255, 255, 0), FILLED, LINE_AA);
                    putText(vis_enh_bgr, cv::format("M[%d,%d]", r, c),
                            Point(cvRound(q.x)+4, cvRound(q.y)-4),
                            FONT_HERSHEY_SIMPLEX, 0.42, Scalar(255,255,0), 1, LINE_AA);
                } else {

                    line(vis_enh_bgr, Point(cvRound(q.x)-4, cvRound(q.y)),
                                       Point(cvRound(q.x)+4, cvRound(q.y)), Scalar(255,255,255), 1, LINE_AA);
                    line(vis_enh_bgr, Point(cvRound(q.x), cvRound(q.y)-4),
                                       Point(cvRound(q.x), cvRound(q.y)+4), Scalar(255,255,255), 1, LINE_AA);
                    putText(vis_enh_bgr, cv::format("F[%d,%d]", r, c),
                            Point(cvRound(q.x)+5, cvRound(q.y)-5),
                            FONT_HERSHEY_SIMPLEX, 0.42, Scalar(255,255,255), 1, LINE_AA);
                }
            }
        }

        cout << "   L2 group " << gi << " 6x2 grid (Anchor -> row=5,col=1, |dx|<=30 => same column; "
                "if col0 missing => col1-56):\n";
        for (int r=0;r<6;++r){
            for (int c=0;c<2;++c){
                const Point2f& q = grid.grid[r][c];
                cout << cv::format("     (%d,%d): (%.1f,%.1f) %s\n", r, c, q.x, q.y,
                                   (grid.is_measured[r][c] ? "[measured]" : "[fitted]"));
            }
        }
    }
    ...
    const string w5 = "Enhanced + L1/L2 + Grid(2x8) + Anchors + 6x2(M/F)";
    ...
    imshow(w2, enhanced16.empty()? view_enhanced : view_enhanced);
    imshow(w3, bin8);
    ...
    setWindowTitle(w5, cv::format(
        "6x2 fit: Anchor=(r5,c1), same-col |dx|<=30, col0-missing=>col1-56 | Grid(2x8): nodes=%zu | L2=%zu | Anchors=%zu | Row-abnormal tol=%.1f",
    ...
    for (size_t i=0;i<grid_nodes_index.size();++i){
        int idx = grid_nodes_index[i].idx;
        if (idx < 0 || idx >= (int)anchors_l2.size()) continue;
        cout << cv::format("   L2 idx=%d -> row=%d col=%d, anchor=(%.1f, %.1f)\n",
                           idx, grid_nodes_index[i].row, grid_nodes_index[i].col,
                           anchors_l2[idx].x, anchors_l2[idx].y);
    }
